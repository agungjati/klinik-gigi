...
<body>
    ...
    <script>
        ...
        function getAllDoctors(search = '', gotoPage = 0) {
            const offset = gotoPage > 0 ? (gotoPage - 1) * limit : 0;
            $.ajax({
                url: '/api/doctors?search=' + search + '&offset=' + offset,
                type: 'GET',
                ...
                success: function(response) {
                    ...
                    for (let i = 0; i < doctors.length; i++) {
                        html += `<tr data-id="${doctors[i].user_id}" >`;
                        html += '<th scope="row">' + (i+1) + '</th>';
                        html += '<td>' + doctors[i].first_name + ' ' + doctors[i].last_name + '</td>';
                        html += '<td>' + doctors[i].email + '</td>';
                        html += '<td>' + doctors[i].phone + '</td>';
                        html += `<td>
                        <button class="btn btn-sm btn-primary" onclick="editModal('${doctors[i].user_id}')" ><em class="fa fa-pencil" />  Edit</button>
                        <button class="btn btn-sm btn-danger" onclick="deleteModal('${doctors[i].user_id}')" ><em class="fa fa-trash" />  Delete</button>
                        </td>`;
                        html += '</tr>';
                    }
                    if(doctors.length == 0) {
                    ...
                    if(doctors.length !== 0) {
                        $('#pagination').html(`
                            <li class="page-item ${currentPage == 1 ? 'disabled' : ''}" >
                                <a class="page-link" href="#" onclick="getAllDoctors('${search}', ${currentPage - 1})" >Previous</a>
                            </li>
                            ${Array.from({length: pages}, (v, k) => k+1).map(page => `
                                <li class="page-item ${currentPage == page ? 'active' : ''}" >
                                    <a class="page-link" href="#" onclick="getAllDoctors('${search}', ${page})" >${page}</a>
                                </li>
                            `).join('')}
                            <li class="page-item ${currentPage == pages ? 'disabled' : ''}" >
                                <a class="page-link" href="#" onclick="getAllDoctors('${search}', ${currentPage + 1})" >Next</a>
                            </li>
                        `);
                    }
                },
        ...
        $(document).ready(function handleUpdate() {
            $('#modal-edit').on('shown.bs.modal', function loadData() {
                let selectedDoctor = doctors.find(doctor => doctor.user_id == selectedUserId);
        ...
        $(document).ready(function handleDelete() {
            ...
            $btnDelete.click(function() {
                ...
                $.ajax({
                    url: '/api/doctors/' + selectedUserId,
                    type: 'DELETE',
                    data: {
                        _token: csrf_token
                    },
